...
SsAnimePack* SsProject::findAnimationPack(SsString& animePackName) {
    SsAnimePack* anime;
    for (SsAnimePackListItr itr = animeList.begin(); itr != animeList.end(); ++itr) {
        anime = itr->get();
...
SsEffectFile* SsProject::findEffect(SsString& effectName) {
    SsEffectFile* effect;
    for (SsEffectFileListItr itr = effectfileList.begin(); itr != effectfileList.end(); ++itr) {
        effect = itr->get();
...
SsSequencePack* SsProject::findSequencePack(SsString& sequencePackName) {
    ...
    for (SsSequencePackListItr itr = sequenceList.begin(); itr != sequenceList.end(); ++itr) {
        sequence = itr->get();
