...
ISSTexture*	SSTextureFactory::loadTexture(SsString filePath)
{
	if (m_myInst)
	{
		...
		if (!cached)
		{
			...
			//SsRawImage* image = new SsRawImage(_tex->getWidth(), _tex->getHeight());
			//image->fill(SsColor(0xff0000ff));

			//delete _tex;
			//_tex = SSTextureFactory::create(image);
			//_tex->loadFromMemory(image->getRaw(), image->getWidth() , image->getHeight() , 4 );
...
ISSTexture* SSTextureFactory::create(SsRawImage* img,SsString Tag)
{
	ISSTexture* _tex = 0;

	_tex = m_myInst->create();
	_tex->filenamepath = Tag;
	_tex->loadFromMemory(img->getRaw(),img->getWidth(),img->getHeight() , 4);
