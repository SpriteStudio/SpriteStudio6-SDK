...
SsAnimeDecoder::SsAnimeDecoder() : curAnimeFPS(0),
...
template <typename mytype>
int SsAnimeDecoder::SsGetKeyValue(SsPart* part, int time, SsAttribute* attr, mytype& value) {
    int useTime = 0;
    ...
    if (lkey == 0) {
        ...
        if (rkey == NULL) {
            ...
            useTime = lkey->time;
            return useTime;
        } else if (lkey->ipType == SsInterpolationType::none) {
            // 補間なし指定なので始点キーの値…
            SsInterpolationValue(time, lkey, 0, value);
            useTime = lkey->time;
            return useTime;
...
static void CoordinateGetDiagonalIntersection(SsVector2& out, const SsVector2& LU, const SsVector2& RU, const SsVector2& LD, const SsVector2& RD) {
    ...
    /* <<< 交差判定 >>> */
    if (((0.0f <= ca) && (1.0f >= ca)) && ((0.0f <= cb) && (1.0f >= cb))) { /* 交差している */
