...
void ViewerMainWindow::valueChanged(Value& value) {
    ...
    } else if (value.refersToSameSourceAs(model->getState()->endFrame)) {
        // endFrameが変更された場合の処理
    ...
    } else if (value.refersToSameSourceAs(model->getState()->length)) {
        // lengthが変更された場合の処理
        buildControlPanel();
        resized();
        view->getState()->length = (int)value.getValue();
    } else if (value.refersToSameSourceAs(model->getState()->fps)) {
        // fpsが変更された場合の処理
        view->getState()->fps = (int)value.getValue();
    } else if (value.refersToSameSourceAs(model->getState()->frame)) {
        // frameが変更された場合の処理
        view->getState()->frame = (int)value.getValue();
    } else if (value.refersToSameSourceAs(model->getState()->animeName)) {
