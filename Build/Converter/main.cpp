...
std::string getFormatExtension(int format) {
    switch ((OutputFormat)format) {
        case OUTPUT_FORMAT_FLAG_SSBP:
            return ".ssbp";
        case OUTPUT_FORMAT_FLAG_JSON:
            return ".json";
        case OUTPUT_FORMAT_FLAG_SSFB:
            return ".ssfb";
        case OUTPUT_FORMAT_FLAG_SSPKG:
            return ".sspkg";
        case OUTPUT_FORMAT_FLAG_CSOURCE:
        default:
            return "";
    }
}
...
struct Options {
    ...
    Options() : isHelp(false), isVerbose(false), endAfterStop(false), encoding(LumpExporter::UTF8), isLogout(false) {
    }
};
...
static std::shared_ptr<Lump> parseParts(spritestudio6::SsProject *proj, const std::string &imageBaseDir, const std::filesystem::path &outPath) {
    ...
    if (!spritestudio6::checkFileVersion(proj->version, SPRITESTUDIO6_SSPJVERSION)) {
        const std::string msg =
            u8"エラー：SpriteStudio Ver.5.xのプロジェクトは使用できません。\n"
            u8"SpriteStudio Ver.6.xで保存する必要があります。";
        throw SSException(msg, SSPC_INCOMPATIBLE_VERSION_ERROR);
    }
    ...
    topLump->add(Lump::stringData((!imageBaseDir.empty()) ? imageBaseDir : "", "imageBaseDir"));
    ...
    if (proj->cellmapList.empty()) {
        const std::string msg = u8"警告：セルマップが存在しません。";
        throw SSException(msg, SSPC_NOT_EXIST_CELLMAP);
    }
    ...
    for (size_t mapIndex = 0; mapIndex < proj->cellmapList.size(); mapIndex++) {
        ...
        if (isZenkaku(&cellMap->name)) {
            const std::string msg = u8"エラー：セルマップに全角が使用されています。半角英数でリネームしてください。: " + cellMap->name;
            throw SSException(msg, SSPC_ZENKAKU_ERROR);
        }

        if (cellMap->cells.empty()) {
            // セルマップにセルが１つも登録されていない場合はエラーにする
            const std::string msg = u8"エラー：セルマップにセルが存在しません。セルを１つ以上作成してください。: " + cellMap->name;
            throw SSException(msg, SSPC_NOT_CELL_IN_CELLMAP);
        }
        for (size_t cellIndex = 0; cellIndex < cellMap->cells.size(); cellIndex++) {
            ...
            if (isZenkaku(&cell->name)) {
                const std::string msg = u8"エラー：セルに全角が使用されています。半角英数でリネームしてください。: " + cell->name;
                throw SSException(msg, SSPC_ZENKAKU_ERROR);
            }
        }
    }
    ...
    if (proj->animeList.empty()) {
        const std::string msg = u8"警告：アニメーションが存在しません";
        throw SSException(msg, SSPC_NOT_ANIMATION);
    }
    ...
    for (int packIndex = 0; packIndex < (int)proj->animeList.size(); packIndex++) {
        ...
        if (isZenkaku(&animePack->name)) {
            const std::string msg = u8"エラー：ファイル名に全角が使用されています。半角英数でリネームしてください。: " + animePack->name;
            throw SSException(msg, SSPC_ZENKAKU_ERROR);
        }
        ...
        for (const auto part : model.partList) {
            ...
            if (isZenkaku(&part->name)) {
                const std::string msg = u8"エラー：パーツ名に全角が使用されています。半角英数でリネームしてください。: " + part->name;
                throw SSException(msg, SSPC_ZENKAKU_ERROR);
            }
            ...
            switch (part->type) {
                                                           ...
                                                           // 参照アニメのポインタが無い場合はNULLパーツになる。
                {
                    ...
                    if (refanime == nullptr) {
                        ...
                        COE(u8"警告：参照のないインスタンスパーツが存在します: " + animePack->name + ".ssae " + part->name);
                    } else {
                        partData->add(Lump::s16Data(part->type, "type"));
                    }
                } break;
                case spritestudio6::SsPartType::effect:  // 5.5エフェクトパーツ
                    ...
                    if (part->refEffectName.empty()) {
                        ...
                        COE(u8"警告：参照のないエフェクトパーツが存在します: " + animePack->name + ".ssae " + part->name);
                    } else {
                        partData->add(Lump::s16Data(part->type, "type"));
                    }
                    break;
                default:
                    ...
                    COE(u8"警告：未対応のパーツ種別が使われています: " + animePack->name + ".ssae " + part->name);
                    partData->add(Lump::s16Data(spritestudio6::SsPartType::null, "type"));
                    break;
            }
        ...
        for (int animeIndex = 0; animeIndex < (int)animePack->animeList.size(); animeIndex++) {
            ...
            for (const auto &partAndAnime : decoder.getPartAnime()) {
                ...
                if (!state) {
                    throw SSException("Not Found", SSPC_INTERNAL_ERROR);
                }
                ...
                for (const auto &partAndAnime : decoder.getPartAnime()) {
                    ...
                    if (part->type == spritestudio6::SsPartType::mesh) {
                        ...
                        if (state->meshPart->uvs == nullptr) {
                            const std::string msg = u8"エラー：メッシュパーツのUV情報が存在しません。ssae:" + animePack->name + " anime:" + anime->name + " part:" + part->name;
                            error_once(part, msg);
                        } else {
                ...
                for (const auto &partAndAnime : decoder.getPartAnime()) {
                    ...
                    if (part->type == spritestudio6::SsPartType::mesh) {
                        ...
                        if (state->meshPart->indices == nullptr) {
                            const std::string msg = u8"エラー：メッシュパーツにメッシュ情報が存在しません。ssae: " + animePack->name + " anime:" + anime->name + " part:" + part->name;
                            error_once(part, msg);
                        } else {
                ...
                for (const auto state : partList) {
                    ...
                    */
                    ...
                    if (state->refAnime) {
                        // インスタンス関連
                        if (
                            (state->instanceValue.curKeyframe != init.instanceValue_curKeyframe) || (state->instanceValue.startFrame != init.instanceValue_startFrame) || (state->instanceValue.endFrame != init.instanceValue_endFrame) || (state->instanceValue.loopNum != init.instanceValue_loopNum) || (state->instanceValue.speed != init.instanceValue_speed) || (state->instanceValue.loopflag != init.instanceValue_loopflag)) {
                    ...
                    if (state->refEffect) {
                        // エフェクト関連
                        if (
                            (state->effectValue.startTime != init.effectValue_startTime) || (state->effectValue.speed != init.effectValue_speed) || (state->effectValue.loopflag != init.effectValue_loopflag) || (state->effectValue.curKeyframe != init.effectValue_curKeyframe)) {
                    ...
                    if (state->is_parts_color) {
                        switch (state->partsColorValue.target) {
                            case spritestudio6::SsColorBlendTarget::whole:
                                if (
                                    (state->partsColorValue.color.rgba.a == 0) && (state->partsColorValue.color.rgba.r == 0) && (state->partsColorValue.color.rgba.g == 0) && (state->partsColorValue.color.rgba.b == 0)) {
                    ...
                    if (state->is_vertex_transform) {
                        for (int vtxNo = 0; vtxNo < 4; vtxNo++) {
                            if (proj->settings.vertexAnimeFloat != 0) {
                                // 小数対応
                                if (
                                    (state->vertexValue.offsets[vtxNo].x != 0.0f) || (state->vertexValue.offsets[vtxNo].y != 0.0f)) {
                                    vt_flags |= 1 << vtxNo;
                                }
                            } else {
                                // 小数点以下切り捨てで
                                if (
                                    ((int)state->vertexValue.offsets[vtxNo].x != 0) || ((int)state->vertexValue.offsets[vtxNo].y != 0)) {
                                    vt_flags |= 1 << vtxNo;
                                }
                            }
                        }
                    ...
                    if (p_flags & PART_FLAG_PARTS_COLOR) {
                        ...
                        if (cb_flags & VERTEX_FLAG_ONE) {
                            frameData->add(Lump::floatData(state->partsColorValue.color.rate,
                                                           tagname + "partsColorValue_color_rate"));
                            frameData->add(Lump::colorData(state->partsColorValue.color.rgba.toARGB(),
                                                           tagname + "partsColorValue_color_rgba"));
                        } else {
                            for (int vtxNo = 0; vtxNo < 4; vtxNo++) {
                                std::string tagname_rate =
                                    tagname + "partsColorValue_colors_" + std::to_string(vtxNo) + "_rate";
                                std::string tagname_rgba =
                                    tagname + "partsColorValue_colors_" + std::to_string(vtxNo) + "_rgba";
                                if (cb_flags & (1 << vtxNo)) {
                                    frameData->add(
                                        Lump::floatData(state->partsColorValue.colors[vtxNo].rate, tagname_rate));
                    ...
                    if (p_flags2 & PART_FLAG_MESHDATA) {
                        ...
                        if (state->meshPart->draw_vertices == nullptr) {
                            error_once(state->part,
                                       "エラー：メッシュパーツに頂点情報が存在しません。ssae: " + animePack->name +
                                           " anime:" + anime->name + " part:" + state->part->name);
                        } else {
                            int i;
            ...
            for (label_idx = 0; label_idx < (int)anime->labels.size(); label_idx++) {
                ...
                // 全角チェック
                if (isZenkaku(&str)) {
                    const std::string msg = u8"エラー：ラベルに全角が使用されています。半角英数でリネームしてください。: " + str;
                    throw SSException(msg, SSPC_ZENKAKU_ERROR);
                }
            ...
            if (isOpenGLContextInitialized()) {
                ConverterOpenGLClear(anime->settings.canvasSize.x, anime->settings.canvasSize.y, anime->settings.pivot.x, anime->settings.pivot.y);
        ...
        for (const auto node : effectmodel->nodeList) {
            ...
            for (const auto &plistindex : behavior.plist) {
                ...
                switch (myType) {
                    ...
                    default:
                        // 未使用のコマンドが含まれている
                        throw SSException(u8"警告：未使用のエフェクトコマンドが含まれています。", SSPC_UNSUPPORT_EFFECT_COMMAND);
                }
            }
        }
    }
...
void convertProject(const std::filesystem::path &outPath, const std::string &outFName, LumpExporter::StringEncoding encoding, const std::filesystem::path &sspjPath, Options &options, const std::string &creatorComment) {
    filelist.clear();
    ...
    if (!std::filesystem::exists(outPath)) {
        const std::string msg = u8"not found output directory: " + outputdirUTF8;
        throw SSException(msg, SSPC_NOT_EXIST_OUTPUT_DIR);
    }
    ...
    if (proj == nullptr) {
        const std::string msg = u8"ssloader_sspj::Load Error";
        throw SSException(msg, SSPC_SSPJ_PARSE_FAILED);
    }
    ...
    try {
        ...
        if (!lump) {
            throw SSException("parseParts error", SSPC_PARTS_ERROR);
        }

        auto outputFilePath = std::filesystem::path(spritestudio6::SsCharConverter::convert_path_string(outPath.string()));
        std::fstream out;

        outputFilePath = outputFilePath / std::filesystem::path(outFName).replace_extension(getFormatExtension(outputFormat));
        out.open(outputFilePath, getFormatMode(outputFormat));
        if (!out.is_open()) {
            const std::string msg = u8"出力ファイルのオープンに失敗しました: " + convert_console_string(outputFilePath.string());
            throw SSException(msg, SSPC_NOT_OUTPUT_FILE);
        }

        COI("convert type : " + OutputFormatToString.at(static_cast<int>(outputFormat)) + " ");
        COI("output file: " + outputFilePath.filename().string());
        if (outputFormat == OUTPUT_FORMAT_FLAG_JSON) {
            ...
            throw SSException(u8"*** OBSOLETE C LANGUAGE SOURCE FORMAT. ***", SSPC_UNSUPPORT_FORMAT);
            ...
            // sspkg に格納する ssfb ファイルの作成
            auto outPathSsfb = std::filesystem::path(sspkg_info::getInst()->get_sspkg_temppath()) / std::filesystem::path(outFName + getFormatExtension(OUTPUT_FORMAT_FLAG_SSFB));
            std::fstream outSsfb;
            outSsfb.open(outPathSsfb, getFormatMode(OUTPUT_FORMAT_FLAG_SSFB));
            LumpExporter::saveSsfb(outSsfb, encoding, lump, creatorComment, s_frameIndexVec);
...
bool parseOption(Options &options, const std::string &opt, ArgumentPointer &args, std::string &illegalArgument) {
    ...
    */
        ...
        if (outputFormat == "json")
            options.outputFormat = OUTPUT_FORMAT_FLAG_JSON;
        else if (outputFormat == "c")
            options.outputFormat = OUTPUT_FORMAT_FLAG_CSOURCE;
        else if (outputFormat == "ssfb")
            options.outputFormat = OUTPUT_FORMAT_FLAG_SSFB;
        else if (outputFormat == "sspkg")
            options.outputFormat = OUTPUT_FORMAT_FLAG_SSPKG;
    } else if (opt == "-c") {
        if (isWindows()) {
...
int convertMain(int argc, const char *argv[]) {
    ...
    if (!success) {
        // 引数が不正
        COE(u8"Invalid arguments: " + illegalArgument + "\n" + std::string(HELP));
    ...
    std::vector<std::string> sources;
    {
        ...
        std::vector<std::string> errorSources;
        for (const auto &str : in) {
            auto inputFile = std::filesystem::path(str);
            if (std::filesystem::exists(inputFile)) {
                sources.push_back(str);
            } else {
                errorSources.push_back(str);
            }
        }
        if (!errorSources.empty()) {
            std::string e;
            for (const auto &a : errorSources) {
                e += " " + a;
            }
            COE("Not found input files:" + e);
    ...
    try {
        ...
        for (auto &source : sources) {
            ...
            if (options.isVerbose) {
                COI("Convert: " + sspjPath.string() + " -> " + outPath.string());
            }
            ...
            if (options.outputFormat == OUTPUT_FORMAT_FLAG_SSPKG) {
                if (!sspkg_info::getInst()->make_sspkg()) {
                    const std::string msg = u8"An error occured during making zip file. " + outPath.string();
